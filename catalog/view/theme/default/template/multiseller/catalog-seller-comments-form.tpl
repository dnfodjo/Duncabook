<h4 id="comment-title"><?php echo $ms_comments_post_comment; ?></h4>

<form id="pcForm">
<b><?php echo $ms_comments_name; ?></b><br />
<input type="text" class="form-control" name="mc_name" value="<?php echo $mc_name; ?>" <?php if ($this->customer->isLogged() && $mscomm_comments_enforce_customer_data) { ?> disabled="disabled" <?php } ?> /><br />
...
<textarea id="mc_text" name="mc_text" class="form-control" cols="40" rows="8" <?php if ($mscomm_comments_maxlen > 0) echo "maxlength='$mscomm_comments_maxlen'" ?>></textarea>
<span style="font-size: 11px;"><?php echo $ms_comments_note; ?></span><br /> <br />

<?php if (!$this->customer->isLogged() || ($this->customer->isLogged() && $this->config->get('mscomm_seller_comments_enable_customer_captcha'))) { ?>
...
<script type="text/javascript">
	var ms_comments_seller_id = <?php echo $seller_id; ?>;
