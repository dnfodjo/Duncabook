<?php
class ControllerModuleMultiMerchComments extends Controller {
	private $data = array();
	...
	public function renderComments() {
        ...
        if(isset($seller_id)){
            $comments_per_page = $this->config->get('mscomm_seller_comments_perpage');
            $query_array = array(
                'seller_id' => $seller_id,
                'displayed' => 1,
            );
            ...
            $comments_per_page = $this->config->get('mscomm_seller_perpage');
            $query_array = array(
        ...
        $pagination->url = $this->url->link('module/multimerch_comments/renderComments', $pagination_url);
        $this->data['pagination'] = $pagination->render();

		list($template, $children) = $this->MsLoader->MsHelper->loadTemplate($comments_template);
		$this->response->setOutput($this->load->view($template, $this->data));
	}
	
	public function renderForm() {
		if (!isset($this->request->get['seller_id']) && !$this->config->get('mscomm_comments_enable')) return;
		if (isset($this->request->get['seller_id']) && !$this->config->get('mscomm_seller_comments_enable')) return;
        ...
        if($seller_id){
            $mscomm_comments_allow_guests = $this->config->get('mscomm_seller_comments_allow_guests');
            $mscomm_comments_enforce_customer_data = $this->config->get('mscomm_seller_comments_enforce_customer_data');
            $mscomm_comments_maxlen = $this->config->get('mscomm_seller_comments_maxlen');
            $comments_template = 'catalog-seller-comments-form';
        }else{
            $mscomm_comments_allow_guests = $this->config->get('mscomm_comments_allow_guests');
            $mscomm_comments_enforce_customer_data = $this->config->get('mscomm_comments_enforce_customer_data');
            $mscomm_comments_maxlen = $this->config->get('mscomm_comments_maxlen');
            $comments_template = 'catalog-comments-form';
        }

		if  (!$this->customer->isLogged() && !$mscomm_comments_allow_guests) {
			...
			$this->data['mscomm_comments_maxlen'] = $mscomm_comments_maxlen;
			$this->data['mscomm_comments_enforce_customer_data'] = $mscomm_comments_enforce_customer_data;
 			$this->data['product_id'] = $product_id;
			$this->data['seller_id'] = $seller_id;
	...
	public function submitComment() {
        ...
        if(isset($comment['seller_id'])){
            if (!$this->config->get('mscomm_seller_comments_enable')) return;

            $mscomm_comments_enforce_customer_data = $this->config->get('mscomm_seller_comments_enforce_customer_data');
            $mscomm_comments_allow_guests = $this->config->get('mscomm_seller_comments_allow_guests');
            $mscomm_comments_maxlen = $this->config->get('mscomm_seller_comments_maxlen');
            $mscomm_comments_enable_customer_captcha = $this->config->get('mscomm_seller_comments_enable_customer_captcha');
        }else{
            if (!$this->config->get('mscomm_comments_enable')) return;

            $mscomm_comments_enforce_customer_data = $this->config->get('mscomm_comments_enforce_customer_data');
            $mscomm_comments_allow_guests = $this->config->get('mscomm_comments_allow_guests');
            $mscomm_comments_maxlen = $this->config->get('mscomm_comments_maxlen');
            $mscomm_comments_enable_customer_captcha = $this->config->get('mscomm_comments_enable_customer_captcha');
        }
		...
		if ($this->customer->isLogged()) {
			...
			if  ($this->config->get('mscomm_comments_enforce_customer_data')) {
			...
			if (!$this->config->get('mscomm_comments_allow_guests')) {
		...
		if (mb_strlen($comment['comment']) > $mscomm_comments_maxlen && $mscomm_comments_maxlen > 0) {
			$json['errors'][] = sprintf($this->language->get('ms_comments_error_comment_long'), $mscomm_comments_maxlen);
		}

		if (!$this->customer->isLogged() || ($this->customer->isLogged() && $mscomm_comments_enable_customer_captcha)) {
