...
class ControllerPaymentMSPPAdaptive extends Controller {
	...
	public function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/ms_pp_adaptive.tpl')) {
			$template = $this->config->get('config_template') . '/template/payment/ms_pp_adaptive.tpl';
		} else {
	...
	public function send() {
		...
		// primary (store)
		$receivers[0] = array();
		$receivers[0]['amount'] = $this->currency->format(0, $order_info['currency_code'], false, false);
		...
		foreach ($order_products as $order_product) {
			...
			if (!isset($receivers[$seller_id])) {
				...
				$receivers[$seller_id]['amount'] = $this->currency->format(0, $order_info['currency_code'], false, false);
			}

			$order_product['total'] += $order_product['tax'];
			//$order_product['total'] += $this->tax->getTax($order_product['price'], $product_data['tax_class_id']);
			...
			if ($order_product['total'] > 0) {
				...
				$store_commission_pct = $this->currency->format($order_product['total'] * $commissions[MsCommission::RATE_SALE]['percent'] / 100, $order_info['currency_code'], false, false);
			...
			// store commission
			//$receivers[0]['amount'] += $this->currency->format($store_commission_flat, $order_info['currency_code'], false, false) + $this->currency->format($store_commission_pct, $order_info['currency_code'], false, false);
			$receivers[0]['amount'] += $this->currency->format($store_commission_flat + $store_commission_pct, $order_info['currency_code'], false, false);
